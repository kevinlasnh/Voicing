...
| 职责 | 负责人 | 范围 |
|---|---|---|
| **前端** | **Claude Code** | 托盘 UI（icon.py / menu.py）、设置小窗口（hotkey_dialog.py / privacy_dialog.py）、视觉状态机（图标 6 态切换）、热键录入交互、跨平台键名归一化 |
| **后端** | **Codex** | ASR 引擎接入（doubao_ime.py / engine.py）、录音（capture.py）、文本注入（injector.py）、全局热键监听（listener.py）、状态机核心逻辑（state.py）、config 读写、日志、单实例保护、credentials 管理、worker 线程模型 |
| **接口契约** | 双方共同遵守 | 前端通过 Qt signal 调后端；后端通过 Qt signal 通知前端状态变化 |

Codex 审查范围：**只审后端方案**（Step 1 环境核实 / Step 3 状态机与数据流 / Step 6 配置与日志 / Step 8 风险 / Step 11 ASR 接口 / Step 12 启动自检 / Step 13 软关闭）。前端方案（Step 4 托盘 UI / Step 5 设置小窗口 / 视觉状态机）由 Claude Code 直接实现。
