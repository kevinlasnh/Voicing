...
- **Status:** in_progress — 功能闭环已锁定，部署计划待 Codex 审查
...
```
┌──────────────────────────────────────┐
│  当前热键：按住 Ctrl+Alt+V           │ ← 状态行（不可点）
│           点击 Ctrl+Alt+B           │
├──────────────────────────────────────┤
│  启用 Voicing              [开]      │ ← 暂停开关（关=热键全失效+图标灰）
├──────────────────────────────────────┤
│  设置按住热键...                      │ ← 打开热键录入弹窗
│  设置点击热键...                      │ ← 打开热键录入弹窗
│  自动 Enter                [关]      │ ← 开关
...
│  打开日志                            │
│  打开配置目录                        │
│  关于 Voicing                        │ ← 版本号 + 第三方声明
│  退出                                │ ← 直接退出，无二次确认
└──────────────────────────────────────┘
```
- 沿用 `pc/voice_coding.py` 的 `ModernMenuWidget` 视觉规范（圆角、悬停高亮、配色）
- PoC 期独立实现，不复制 `pc/` 代码
...
**入口（托盘菜单提供 2 个独立入口）**：
- 「设置按住热键…」→ 打开 `HotkeyDialog(target="hold")`
- 「设置点击热键…」→ 打开 `HotkeyDialog(target="toggle")`

**两个热键的语义差异（视觉上要让用户一眼看懂）**：

| 维度 | 按住热键（hold） | 点击热键（toggle） |
|---|---|---|
| 触发动作 | 按下 KeyDown 开始录音 | 按一下 KeyUp 开始录音 |
| 结束动作 | 松开 KeyUp 立刻发送 | 再按一下 KeyUp 才发送 |
| 适用场景 | 短句快说（聊天、命令、快速搜索） | 长段口述（写邮件、写文档） |
| 默认值 | `Ctrl+Alt+V` | `Ctrl+Alt+B` |
| 弹窗标题文案 | 「设置「按住说话」热键」 | 「设置「点击切换」热键」 |
| 弹窗副文案 | 「按住此键期间录音，松开自动发送」 | 「按一下开始录音，再按一下结束并发送」 |

**用户录入完整流程**：

```
1. 用户右键托盘 → 看到顶部状态行：
   「按住热键：Ctrl+Alt+V    点击热键：Ctrl+Alt+B」
...
3. 弹窗 modal 居中展示：
   ...
   │  └────────────────────────────┘          │
   │                                            │
   │  规则：必须含 Ctrl/Alt/Shift/Win 至少一个  │
   │       不能与「点击热键」(Ctrl+Alt+B) 冲突   │
   │                                            │
   │       [取消 (Esc)]    [恢复默认]  [保存]   │
   └──────────────────────────────────────────┘

4. 用户按下 Ctrl+Alt+M（举例）
   - 弹窗实时显示「Ctrl + Alt + M」
   - 用户继续按（覆盖）或松开（确认）

5. 用户全部松开后，弹窗显示捕获到的最终组合：「Ctrl + Alt + M」
   - 旁边出现绿色 ✓ 或红色 ✗
   - ✓：组合合法且不与另一个热键冲突
   - ✗：显示具体原因（如「与点击热键冲突」「单字母键不允许」）

6. 用户点「保存」
   ├ 校验通过 → 卸载旧 hold 热键 → 注册新 hold 热键
   │   ├ 系统级注册成功 → config.json 落盘 → 弹窗关闭 → 状态行更新
   │   └ 系统级注册失败（被其他软件占用）→ 回滚旧热键 → 弹窗显示红字「注册失败：可能被其他软件占用」+ 不关闭弹窗
   └ 校验失败 → 弹窗显示红字提示原因 + 不关闭弹窗

7. 用户点「恢复默认」→ 弹窗内填入默认值（不立即保存，仍需点保存）

8. 用户点「取消」或按 Esc → 弹窗关闭，config 不变
```

**两个弹窗的隔离**：

- 同一时刻只允许打开一个 HotkeyDialog（modal，互斥）
- 弹窗打开期间：**完全卸载主热键 hook**（`keyboard.unhook_all()`），避免用户按到当前的 hold/toggle 键时误触录音
- 弹窗关闭时：**重新注册两个主热键**（不是只注册被改的那个，避免状态不一致）
- 弹窗打开前置条件：`state == IDLE`，否则托盘菜单的「设置...」入口灰化（避免录音中改键的混乱）

**冲突校验时机**：

- **录入实时阶段**：每次按键变化，立刻校验「是否单键」「是否系统占用」，红✗ 提示
- **点保存时**：再校验「是否与另一个热键有子集关系」（hold ⊂ toggle 或 toggle ⊂ hold）
- **OS 注册时**：调 `keyboard.add_hotkey()` 真正注册，注册返回 false 才算最终失败

**键名归一化展示**：

- 内部存储：`["ctrl", "alt", "v"]`（小写，按字母排序）
- Windows / Linux 显示：`Ctrl + Alt + V`
- macOS 显示：`⌃ ⌥ V`（Unicode 符号）
- 弹窗实时显示用「+ 」分隔的友好格式

**键名映射表（跨平台关键键）**：

| 内部名 | Windows | macOS | Linux |
|---|---|---|---|
| `ctrl` | Ctrl | ⌃ Control | Ctrl |
| `alt` | Alt | ⌥ Option | Alt |
| `shift` | Shift | ⇧ Shift | Shift |
| `win` / `cmd` | Win | ⌘ Command | Super |
| 字母键 | V | V | V |
...
#### Step 4：托盘 UI 实现（pending）

- 图标资源：复用 `pc/` 现有圆形图标，按 6 态生成预缓存
- 菜单：按 0.2 结构实现，沿用 ModernMenuWidget 视觉规范
- 「启用 Voicing」开关：切换 PAUSED ↔ IDLE，立即生效
- 「自动 Enter」/「标点」/「开机自启」：切换 config.json 对应字段，立即生效
- 「设置按住热键...」/「设置点击热键...」：打开 HotkeyDialog（见 Step 5）
- 「打开日志」：系统默认应用打开日志文件
- 「打开配置目录」：打开 `%APPDATA%/Voicing/desktop/`
- 「关于 Voicing」：弹窗显示版本号 + 第三方声明
- 「退出」：直接退出，无二次确认
- tooltip：实时显示当前状态 + 当前热键

**验收**：Windows 真机托盘菜单弹出、所有项点击响应正确、状态行实时更新

---

#### Step 5：热键录入弹窗（pending）

**前置约束**：弹窗打开前 `state` 必须 `== IDLE`，否则托盘菜单灰化「设置...」入口。

弹窗布局（无标题栏 modal QDialog）：
```
┌──────────────────────────────────────┐
│                                       │
│      请按下您想要的快捷键              │
...
录入状态机：
```
WAITING   ── KeyDown ──→  CAPTURING（实时显示）
CAPTURING ── 全部释放 ──→ CAPTURED
CAPTURED  ── 点保存 ──→   VALIDATE → SAVE / REJECT
CAPTURED  ── 继续按 ──→   CAPTURING（覆盖）
ANY       ── Esc ──→      CANCEL
```
...
- 拒绝：与另一个热键有**子集关系**（A ⊂ B 当且仅当 set(A.keys) ⊆ set(B.keys)；包括相同情况）
  - 例：hold=`Ctrl+V`，toggle=`Ctrl+V+B` → 拒绝（hold ⊂ toggle）
  - 例：hold=`Ctrl+Alt+V`，toggle=`Ctrl+Shift+V` → 通过（无包含关系）
- 拒绝：OS 注册失败

跨平台键名归一化：
- 内部存储：`["ctrl", "alt", "v"]`（字母排序）
- 显示：Windows `Ctrl+Alt+V` / macOS `⌃⌥V`
- 保存到 config.json

保存后即时生效：卸旧 → 注新 → 失败回滚 → 状态行更新

**验收**：录入 Ctrl+Alt+V 生效 / 纯字母被拒 / 两个热键设相同被拒 / Esc 取消不改 config
