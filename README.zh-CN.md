...
- **Linux 终端感知粘贴** — GNOME Wayland 自动粘贴会对普通输入框发送 Ctrl+V，对终端焦点发送 Ctrl+Shift+V
...
Linux/GNOME Wayland 日常使用保持桌面端"自动粘贴"即可。自动粘贴会对普通输入框发送 Ctrl+V，检测到当前焦点是终端时自动切到 Ctrl+Shift+V。只有遇到某个终端或特殊软件没有识别正确时，才需要手动切换到"终端粘贴"或"兼容粘贴"。
...
10. GNOME Wayland 默认使用自动粘贴：普通输入框发送 Ctrl+V，通过 AT-SPI 检测到终端焦点时发送 Ctrl+Shift+V；托盘菜单可手动切换为普通、终端或兼容粘贴模式
