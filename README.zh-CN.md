...
| Linux | `voicing-linux-x86_64` | Ubuntu 22.04+ GNOME X11，或具备 RemoteDesktop portal 的 GNOME Wayland |
...
- **Linux**：`chmod +x voicing-linux-x86_64` 后运行，开启 Wi-Fi 热点或连接同一局域网。GNOME Wayland 首次粘贴文字时可能会要求授权 RemoteDesktop 键盘会话
...
9. 桌面端通过剪贴板粘贴文本，并在需要时补发一次 Enter；Linux X11 走常规 Ctrl+V 路径，GNOME Wayland 走 RemoteDesktop portal 键盘授权，并可在自动、普通、终端和兼容粘贴模式之间切换
...
6. 如果电脑端是 macOS，请确认已更新并重启到 v2.9.4，这一版不再因为 `en0/en1` 网卡命名误判而优先发布错误地址
