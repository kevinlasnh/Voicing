...
- **Linux 终端感知粘贴** — GNOME Wayland 自动粘贴会对确认的普通输入框发送 Ctrl+V，对终端或无法稳定确认的焦点发送 Ctrl+Shift+V
...
Linux/GNOME Wayland 日常使用保持桌面端"自动粘贴"即可。自动粘贴会对确认的普通输入框发送 Ctrl+V，检测到当前焦点是终端时自动切到 Ctrl+Shift+V。如果 GNOME 焦点检测暂时无法稳定确认，自动粘贴会优先走终端安全的 Ctrl+Shift+V；如果某个普通软件因此需要普通粘贴，再手动切到"普通粘贴"。
...
10. GNOME Wayland 默认使用自动粘贴：确认的普通输入框发送 Ctrl+V，终端或无法稳定确认的焦点发送 Ctrl+Shift+V；托盘菜单可手动切换为普通、终端或兼容粘贴模式
