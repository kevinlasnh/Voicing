...
## 36. 前端审视：v2.7.2 设计 token 提取 + Desktop 版风格一致性（2026-04-22）

从 `pc/voice_coding.py:660-970` 提取的 v2.7.2 `ModernMenuWidget` 设计 token：
...
Desktop 版必须完全复用以上 token，包括设置小窗口。

业界标准合规：
- Windows 11 Fluent Design：暗色 Mica/Acrylic 背景、圆角 8px、subtle 边框 → 对齐
- macOS HIG Menu Bar Extra：菜单 + 设置走独立窗口 → 对齐
- 热键录入交互：点按钮 → "Press any key" → 捕获 → 显示 → 即时注册（Discord / ShareX 同款）→ 对齐

---

## 37. 热键录入方案决策：方案 A 临时设置小窗口（2026-04-22）

调研结论：业界几乎所有需要自定义热键的托盘软件都会开设置窗口。
- Discord PTT：设置窗口 → "Record Keybind" 按钮
- ShareX：专门的 Hotkey Settings 窗口
- Snipaste：首选项窗口
- PowerToys：每个模块独立设置窗口

"纯托盘 + 自定义热键 + 不弹任何窗口"在业界不存在。

用户确认接受方案 A：弹临时 preference dialog（约 420×380，暗色 Fluent 风格，用完即关）。
这不是"Desktop 主窗口"，是设置对话框，与"纯托盘"定位兼容。
...
| 职责 | 负责人 | 范围 |
|---|---|---|
| 前端 | Claude Code | 托盘 UI / 设置小窗口 / 视觉状态机 / 热键录入交互 / 跨平台键名归一化 |
| 后端 | Codex | ASR 引擎 / 录音 / 文本注入 / 全局热键监听 / 状态机核心 / config / 日志 / 单实例 / credentials / worker 线程 |
| 接口 | 共同 | 前端通过 Qt signal 调后端；后端通过 Qt signal 通知前端状态变化 |
