...
# 2026-09-25 起 Wayland 输入统一走 Ctrl+V。
# 历史背景：v2.9.5 ~ v2.9.9 曾用 AT-SPI 采样投票判断当前焦点是终端还是普通窗口，再决定发
# Ctrl+V（普通窗口）还是 Ctrl+Shift+V（终端）。该判定在 GNOME Wayland 冷启动、GNOME Shell
# 抢占焦点、辅助功能总线未就绪等场景下会误判，而且每次粘贴最多要等 500ms 采样窗口。
# 用户确认不再使用终端内的 agent，终端粘贴场景整体移除，因此这里彻底删掉了 AT-SPI 焦点探测、
# terminal 名称名单、terminal 焦点缓存和粘贴模式枚举，避免死代码继续误导后续维护。
# 万一将来要恢复终端粘贴，请从 git 历史取回 v2.9.9 的 pc/platform_keyboard.py。
...
class RemoteDesktopPortalKeyboardBackend:
    ...
    def paste_from_clipboard(self) -> None:
        # Wayland 下固定发送 Ctrl+V：不再做焦点探测，普通窗口与终端共用同一快捷键。
        self._ensure_started()
        self._send_key_sequence(_ctrl_v_sequence())
...
def _ctrl_v_sequence() -> tuple[tuple[int, int], ...]:
    """Wayland portal 唯一使用的粘贴序列：Ctrl+V。

    2026-09-25 起不再按焦点在 Ctrl+V / Ctrl+Shift+V 之间切换，
    原因见文件顶部 KEYSYM 常量区的改动说明。
    """
    return (
