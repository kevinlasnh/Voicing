...
class PlatformKeyboardTests(unittest.TestCase):
    # 2026-09-25：原 setUp 每次重置 AT-SPI terminal 焦点缓存；终端探测删除后已无缓存可重置。
    ...
    def test_wayland_paste_never_probes_focus_or_spawns_subprocess(self):
        """回归保护：Wayland 粘贴必须保持纯 Ctrl+V 路径。

        2026-09-25 之前该路径会 fork 系统 Python 去采集辅助功能树（每次最多 500ms），
        并据此在 Ctrl+V / Ctrl+Shift+V 之间切换；现在不允许再出现任何探测子进程。
        """
        backend = platform_keyboard.RemoteDesktopPortalKeyboardBackend()
        with patch.object(backend, "_ensure_started"):
            with patch.object(backend, "_send_key_sequence") as mock_send:
                with patch("platform_keyboard.subprocess.run") as mock_run:
                    with patch("platform_keyboard.subprocess.check_output") as mock_check:
                        backend.paste_from_clipboard()
        mock_send.assert_called_once_with(platform_keyboard._ctrl_v_sequence())
        mock_run.assert_not_called()
        mock_check.assert_not_called()

    def test_paste_mode_api_is_fully_removed(self):
        """粘贴模式概念已从 platform_keyboard 移除，防止文档或托盘菜单再次引用已删除行为。"""
        for name in (
            "PasteMode",
            "get_paste_mode",
            "set_paste_mode",
            "get_paste_mode_label",
            "is_current_focus_terminal",
            "_resolve_wayland_paste_sequence",
        ):
            self.assertFalse(hasattr(platform_keyboard, name), f"{name} 应已被删除")
