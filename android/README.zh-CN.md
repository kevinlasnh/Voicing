...
7. Linux/GNOME Wayland 桌面端日常保持托盘里的"自动粘贴"即可；自动粘贴在焦点检测不稳定时会优先走终端安全的 Ctrl+Shift+V，只有个别软件需要时再手动切到"普通粘贴"或"兼容粘贴"。如果 GNOME 在登录或 Voicing 启动后弹出 RemoteDesktop 键盘授权，请先允许再发送文字
